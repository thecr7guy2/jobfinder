...
class ABNAMROScraper(BaseScraper):
    source = "abn_amro_api"

    def fetch_raw_jobs(self) -> list[dict[str, Any]]:
        response = self.session.get(
            str(self.company["api_url"]),
        ...
        jobs = list(response.json().get("vacancies", []))
    ...
    def normalize_job(self, raw_job: Mapping[str, Any], fetched_at: str) -> dict[str, Any]:
        ...
        description = html_to_text(str(raw_job.get("detail_description") or raw_job.get("intro") or ""))
        ...
        return {
            ...
            "posted_date": iso_date(
                str(raw_job.get("detail_posted_date") or raw_job.get("created") or raw_job.get("updated") or "")
            ),
    ...
    def _query_params(self) -> list[tuple[str, Any]]:
    ...
    def _detail_url(self, raw_job: Mapping[str, Any]) -> str:
        ...
        template = str(
            self.company.get("job_url_template") or "https://www.werkenbijabnamro.nl/en/vacancy/{id}/{slug}"
        )
        if slug:
            return template.format(id=raw_id, slug=slug)
        return f"https://www.werkenbijabnamro.nl/en/vacancy/{raw_id}"
